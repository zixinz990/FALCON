...
class BasePolicy:
    ...
    def _init_joystick_handler(self):
        ...
        if sys.platform == "darwin":
            ...
            self.wc_key_map = {
                ...
                32: "L2",
                64: "F1",
                128: "F2",  # F1, F2 not used in sim2sim
                256: "A",
                512: "B",
                768: "A+B",
                1024: "X",
            ...
            if self.sdk_type == "unitree":
                ...
                self.wireless_controller_subscriber.Init(
                    self.wireless_controller_handler, 1
                )
            else:
                raise NotImplementedError(
                    f"Joystick is not supported for {self.sdk_type} SDK."
                )
    ...
    def get_current_obs_buffer_dict(self, robot_state_data):
        ...
        current_obs_buffer_dict["dof_pos"] = (
            robot_state_data[:, 7 : 7 + self.num_dofs] - self.default_dof_angles
        )
    ...
    def parse_current_obs_dict(self, current_obs_buffer_dict):
        ...
        for key in self.obs_dict:
            ...
            current_obs_dict[key] = np.concatenate(
                [
                    current_obs_buffer_dict[obs_name] * self.obs_scales[obs_name]
    ...
    def get_init_target(self, robot_state_data):
        ...
        if self.get_ready_state:
            ...
            q_target = dof_pos + (self.default_dof_angles - dof_pos) * (
                self.init_count / 500
            )
            self.init_count += 1
            return q_target
            ...
            if scaled_policy_action.shape[1] != self.num_dofs:
                if not self.upper_body_controller:
                    scaled_policy_action = np.concatenate(
                        [
                            np.zeros(
                                (1, self.num_dofs - scaled_policy_action.shape[1])
                            ),
    ...
    def process_joystick_input(self):
        ...
        if self.wc_msg.keys == 0:
            self.lin_vel_command[0, 1] = (
                -(self.wc_msg.lx if abs(self.wc_msg.lx) > 0.1 else 0.0)
                * self.stand_command[0, 0]
            )
            self.lin_vel_command[0, 0] = (
                self.wc_msg.ly if abs(self.wc_msg.ly) > 0.1 else 0.0
            ) * self.stand_command[0, 0]
            self.ang_vel_command[0, 0] = (
                -(self.wc_msg.rx if abs(self.wc_msg.rx) > 0.1 else 0.0)
                * self.stand_command[0, 0]
            )
