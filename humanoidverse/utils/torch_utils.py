...
def generate_sphere_sample_params(
    ...
    Returns:
        Tuple[Tensor, Tensor]:
            - unit_dirs: (N, 3) unit direction vectors
            - radius_scales: (N, 1) scale factors for radius
    """
    rand_vec = torch.randn(N, 3, device=device)
    norm = torch.norm(rand_vec, dim=-1, keepdim=True)
    unit_dirs = rand_vec / (norm + 1e-8)
...
def apply_sphere_sample_to_segments(
    ...
    Args:
        start (Tensor): (N, 3) segment start points.
        end (Tensor): (N, 3) segment end points.
        unit_dirs (Tensor): (N, 3) unit direction vectors.
        radius_scales (Tensor): (N, 1) scale factors in [0,1] for radius.

    Returns:
        Tensor: (N, 3) sampled points inside the spheres.
    """
...
def sample_3d_directions(num_samples: int, device: str = "cuda:0") -> torch.Tensor:
    ...
    Args:
        ...
        device (str): Device to allocate the tensor on.
    Returns:
        torch.Tensor: A tensor of shape (num_samples, 3) containing the sampled directions.
    """
    # Generate random points in 3D space
    rand = torch.randn(num_samples, 3, device=device)
